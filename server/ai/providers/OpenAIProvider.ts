    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          endDate: { type: 'string', description: 'Target end date YYYY-MM-DD for goals or plans.' },
          targetGoal: { type: 'string', description: 'Detailed strategy or goal motivation.' },
          targetMetric: { type: 'string', description: 'Unit of measurement (e.g. متابع, كتاب, كيلو, %)' },
          targetValue: { type: 'number', description: 'Numeric target objective to reach (e.g. 1000, 10)' },
          milestones: { type: 'array', items: { type: 'string' }, description: 'Array of sequential phase titles or milestone steps for the goal.' },
          imageUrl: { type: 'string', description: 'Image URL or attached media link.' },
          isLongNote: { type: 'boolean', description: 'True if long note, poem, excerpt, or article.' },
        },
        required: ['type', 'title'],
      },
    },
