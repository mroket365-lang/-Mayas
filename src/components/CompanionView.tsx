...
export const CompanionView: React.FC<CompanionViewProps> = ({
  ...
  // Auto-trigger Focus Mode when microphone is recording or typing > 30s
  const typingTimerRef = useRef<any>(null);

  useEffect(() => {
    if (isListening && !isFocusMode && onToggleFocusMode && isFeatureEnabled('feature_focus_mode')) {
      onToggleFocusMode();
    }
  }, [isListening, isFocusMode, onToggleFocusMode, isFeatureEnabled]);

  useEffect(() => {
    if (inputText.trim().length > 0 && !isFocusMode) {
      if (!typingTimerRef.current) {
        typingTimerRef.current = setTimeout(() => {
          if (onToggleFocusMode && isFeatureEnabled('feature_focus_mode')) {
            onToggleFocusMode();
          }
    ...
    return () => {
      if (typingTimerRef.current && (inputText.trim().length === 0 || isFocusMode)) {
        clearTimeout(typingTimerRef.current);
        typingTimerRef.current = null;
      }
    };
  }, [inputText, isFocusMode, onToggleFocusMode, isFeatureEnabled]);
