...
import { getTaskCategoryConfig } from '../constants/taskCategories';
...
export const TaskItemCard: React.FC<TaskItemCardProps> = ({
  ...
  const categoryConfig = getTaskCategoryConfig(item.category, isArabic);
      ...
      <div className="flex items-center justify-between gap-2.5 p-3 sm:px-4 sm:py-3 min-h-[64px]">
        ...
        >
          ...
          <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
            ...
            {categoryConfig && (
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold border shrink-0 transition-colors ${categoryConfig.badgeClass}`}
                title={isArabic ? `تصنيف: ${categoryConfig.nameAr}` : `Category: ${categoryConfig.nameEn}`}
              >
                <span className={`w-1.5 h-1.5 rounded-full ${categoryConfig.dotClass}`} />
                <span>{categoryConfig.icon}</span>
                <span>{isArabic ? categoryConfig.nameAr.split(' ')[0] : categoryConfig.nameEn.split(' ')[0]}</span>
              </span>
            )}
          </div>

          {/* Small Subtitle / Description & Meta Tags */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 text-[11px] text-[var(--text-muted)] font-medium leading-normal">
            {item.description ? (
              <p className="line-clamp-2 text-xs text-[var(--text-muted)] font-normal whitespace-pre-line leading-relaxed">
                {item.description}
              </p>
            ) : null}
                ...
                {editingSubtaskId === st.id ? (
                  <div className="flex items-center gap-1.5 flex-1">
                    <input
                      ...
                      className="flex-1 px-2 py-1 rounded-lg border border-indigo-500 bg-[var(--bg-main)] text-xs text-[var(--text-main)] font-semibold focus:outline-none"
                      autoFocus
                    />
                    <button
                      onClick={() => handleSaveEditedSubtask(st.id)}
                      className="px-2 py-1 rounded-lg bg-indigo-600 text-white text-[10px] font-bold hover:bg-indigo-700 transition-all shrink-0"
                    >
                      {isArabic ? 'حفظ' : 'Save'}
                    </button>
                    <button
                      ...
                      className="px-2 py-1 rounded-lg bg-slate-700 text-slate-200 text-[10px] font-bold hover:bg-slate-600 transition-all shrink-0"
                    >
                  ...
                  <>
                    <button
                      ...
                      className="flex items-start gap-2 flex-1 text-right min-w-0"
                    >
                      {st.completed ? (
                        <CheckSquare className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />
                      ) : (
                        <Square className="w-3.5 h-3.5 text-[var(--text-muted)] shrink-0 mt-0.5" />
                      )}
                      <span className="text-xs font-semibold leading-relaxed break-words whitespace-pre-line text-[var(--text-main)]">
                        {st.title}
                      </span>
                    </button>
                    ...
                    <div className="flex items-center gap-1 shrink-0 ms-1">
                      <button
                        ...
                        className="p-1 rounded text-[var(--text-muted)] hover:text-indigo-500 transition-colors"
                        title={isArabic ? 'تعديل الخطوة' : 'Edit subtask'}
                      >
                      ...
                      <button
                        ...
                        title={isArabic ? 'حذف الخطوة' : 'Delete subtask'}
                      >
